...
def four_level_system_dynamics(end_time=10.0):
    ...
    v_gamma_up = 0.0244+0.0555j  # from calc_gamma.py
    v_gamma_down = 1.63e-06-0.0193j
    ...
    tau_x = (tau_plus + tau_minus) / np.sqrt(2)
    ...
    gammas = [lambda t: v_gamma_down, lambda t: v_gamma_up]
    lindblad_operators = [lambda t: np.sqrt(v_gamma_down) * (tau_minus + sqrt_s * np.matmul(sigma_plus, sigma_minus)),
